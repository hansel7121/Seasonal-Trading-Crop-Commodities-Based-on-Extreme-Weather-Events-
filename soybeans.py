...
df = pd.read_csv("crops_data/iowa_soybean_temps_10y.csv", index_col="Date", parse_dates=True)
...
def get_soybean_buy_signals():
    """Calculate soybean buy signals without displaying plots"""
    extreme_hots = []
    extreme_colds = []
    for i in range(len(df)):
    ...
    for date in all_dates:
        if date not in soybean_prices.index or date.year == 2025:
            continue
        month_key = (date.year, date.month)
        if month_key in seen_months:
            continue
...
if __name__ == "__main__":
    ...
    soybean_buy_signals = buy_signals(extreme_hots, extreme_colds, soybean_prices)
    cash, annual_returns = plot_returns(soybean_prices, soybean_buy_signals, 6)
